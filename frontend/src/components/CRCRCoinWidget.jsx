...
const CRCRCoinWidget = ({ compact = false, navigateOnClick = false }) => {
  ...
  const onDaily = async () => {
    ...
    try {
      ...
      if (res?.success) {
        toast.success(`Á∞ΩÂà∞?êÂ?ÔºÅÁç≤Âæ?${res.amount} CRCRCoin`)
      } else {
        const msg = res?.error || \u0027©|•º•i¶A¶∏√±®Ï\u0027`r`n        toast.error(msg)
      }
    } finally {
      setClaiming(false)
    }
  ...
  // ?çÁ´Ø?êÁ§∫?®Ô?‰º∞Á?‰∏ã‰?Ê¨°Á∞Ω?∞Á??†Ê?ÔºàÂØ¶?õ‰ª•‰º∫Ê??®ÁÇ∫Ê∫ñÔ?
  const nextGrant = (() => {
    try {
      const base = 50
          ...
          <div className="bg-white/80 backdrop-blur-xl border border-white/25 rounded-2xl shadow-xl">
            ...
            <div className="flex items-center justify-between p-4 pb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-gray-900">CRCRCoin ?¢Â?</h3>
              </div>
              <button
            ...
            <div className="px-4 pb-4">
              <div className="rounded-xl border border-white/25 bg-gradient-to-br from-white/70 to-white/50 p-4">
                <div className="text-xs text-gray-600">?ÆÂ?È§òÈ?</div>
            ...
            <div className="px-4 pb-2">
              <div className="text-[11px] text-gray-600 mb-1">
                ≥sƒÚ√±®Ï°G<span className="font-semibold text-yellow-700">{Math.max(0, streak || 0)}</span> §—
              </div>
              <div className="flex items-center justify-between">
                <div className="text-xs text-gray-700">
                  ®C§È√±®Ïº˙¿y°G<span className="font-semibold text-yellow-700">+{nextGrant.total}</span>
                  <span className="text-gray-500 ml-1">(ßt•[¶® +{nextGrant.bonus})</span>
                </div>
                <button
                  type="button"
                  onClick={onDaily}
                ...
                >
                  {!isLoggedIn
                    ...
                    : (!hydrated
                        ...
                        : (claiming
                            ? '√±®Ï§§..'
                            : (canClaimNow ? '√±®Ï' : `µ•≠‘ ${fmtTime(leftMs)}`)))}
                </button>
              </div>
              {!isLoggedIn && (
            ...
            <div className="px-4 pb-4">
              <div className="text-xs font-semibold text-gray-800 mb-2">?ÄËøëÁ???/div>
              {history.length === 0 ? (
                <div className="text-xs text-gray-500 py-3">Â∞öÁÑ°‰∫§Ê?Á¥Ä??/div>
              ) : (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-gray-700">
                          {h.reason || (h.type === 'claim' ? 'ÊØèÊó•Á∞ΩÂà∞' : h.type === 'earn' ? '?≤Â?' : 'Ê∂àË≤ª')}
                        </span>
                      </div>
                      <div
                        className={[
